...
#(a)
library("faraway")
scatterplot3d(x=diabetes$hip,
              y=diabetes$waist,
              z=diabetes$weight,
              highlight.3d=TRUE,
              pch=c(1,2)[as.numeric(diabetes$gender)],
              xlab="Hip",
              ylab="Waist",
              zlab="Weight")
legend("topleft",
       legend=levels(diabetes$gender),
       pch=1:2)
#(b)
aq <- na.omit(airquality)
ozcols <- topo.colors(50)
colindex <- cut(aq$Ozone,
                breaks=seq(min(aq$Ozone),
                           max(aq$Ozone),
                           length=51),
                include.lowest=TRUE)
scatterplot3d(x=aq$Wind,
              y=aq$Solar.R,
              z=aq$Temp,
              type="h",
              lty.hplot=3,
              color=ozcols[colindex],
              pch=aq$Month-4,
              main="NY Air Quality",
              xlab="Wind (MPH)",
              ylab=expression(paste("Solar Radiation (",
                                    ring(A),
                                    ")")),
              zlab="Temperature (F)")
legend(locator(1),
...
nuc.fit1 <- lm(cost~cap+date,data=nuclear)
nuc.fit2 <- lm(cost~cap*date,data=nuclear)
capseq <- seq(min(nuclear$cap),
              max(nuclear$cap),
              length=50)
datseq <- seq(min(nuclear$date),
              max(nuclear$date),
              length=50)
capdat <- expand.grid(cap = capseq, date = datseq)
nuc.pred1 <- matrix(predict(nuc.fit1, newdata = capdat), 50, 50)
...
persp(
      ...
      z = nuc.pred1,
      theta = 25,
      zlim = range(c(nuc.pred1, nuc.pred2)),
      ticktype = "detailed",
      xlab = "Capacity",
...
#' As expected based on earlier results, there's barely any visual 
#' differences between the two surfaces. Inclusion of the interaction 
#' term, as evidenced by extremely large p-values, has no tangible 
#' effect on modeling the response.
